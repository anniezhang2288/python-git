...
"""
Three types of objects in Git: 
    1. blobs (ordinary files)
    2. commits 
    3. trees (represent state of a single directory)
...
# finds object by hash (or hash prefix)
...
# inverse of find_object: reads an object and its type
...
# pretty-prints an object's contents (or its size or type) to stdout
...
"""ls_files prints all files in the index (if -s is specified then it prints with their mode)"""
def get_status():
